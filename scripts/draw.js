...
function mainDraw() {
    ...
    program = initShaders(gl, "vshaderDraw", "fshaderDraw");
    ...
    window.onkeypress = function(event) {
        ...
        switch (event.key) {
            ...
            case 'c': //Toggle the current color being drawn to the screen (Black, Red, Green, Blue)
                ...
                if (mode == 'file') {
                    openFileDraw(openEvent); //Redraw the file if in file mode
                }
                break;
        }
    }
...
/*
 * renderDraw() - Creates the necessary buffers, projections and transformation matrices
...
function drawMode(event){
    ...
    let offset = getOffset(gl.canvas);
    //Check if the user has clicked within the canvas space
    if(event.clientX < gl.canvas.width + offset.left && event.clientY < gl.canvas.height + offset.top){
        userSegs[polyCnt-1].push(vec4(event.clientX - offset.left, event.clientY - offset.top, 0.0, 1.0)); //Add the mouse position to the user segment array
